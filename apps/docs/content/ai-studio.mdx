...
The origin must match the hosted studio you're on (`https://app.lerret.io`); Lerret's in-app guide shows the exact command to copy.
...
**Already shipped, outside the studio:** the [Claude Code plugin](/ai-editing/#the-claude-code-plugin) — `/lerret` (v0.2.3) from the Belikely United marketplace. It's a different shape from the in-studio agent: the plugin uses your own Claude Code session as the brain, so there's no API key and no subscription to Lerret, while the in-studio agent runs in the browser with your own provider key or a local Ollama. It's Claude Code-only and complements the studio agent rather than replacing it. See [Editing with AI Tools](/ai-editing/).

Still deferred to a later release: animated / GIF export, and any background or server-side agent. There is no managed-subscription Lerret AI on the roadmap either — Lerret has no backend, by design.
