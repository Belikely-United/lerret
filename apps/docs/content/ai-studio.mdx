...
**Already shipped, outside the studio:** the Claude Code plugin — `/lerret` (v0.2.3) from the Belikely United marketplace. It's keyless: no API key and no subscription to Lerret, because your own Claude does the work. It's Claude Code-only and complements the studio agent rather than replacing it. See [Editing with AI Tools](/ai-editing/).
